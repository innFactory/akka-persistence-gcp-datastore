...
import akka.persistence.datastore.journal.read.sources.{
  CurrentPersistenceIdsSource,
  PersistenceEventsByPersistenceIdSource,
  PersistenceEventsByTagSource,
  PersistenceIdsSource
}
...
class DatastoreScaladslReadJournal(system: ExtendedActorSystem, config: Config)
    extends ReadJournal
